...
public sealed class AuthUserRepository : IAuthUserRepository
{
    ...
    public AuthUserRepository(AuthDbContext db) { _db = db; UnitOfWork = new EfUnitOfWork(db); }
    ...
    public Task<AuthUser?> GetByIdAsync(TenantId tenantId, int id, CancellationToken ct)
        => _db.Users.Include(u => u.Claims).FirstOrDefaultAsync(u => u.Id == id && u.TenantId == tenantId && !u.IsDeleted, ct);
