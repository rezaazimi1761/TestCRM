...
public sealed class UsersController(AuthDbContext db):ControllerBase
{
 ...
 public IActionResult GetAll(int page=1,int pageSize=20,string? sortBy=null,bool sortDesc=false,string? search=null,string? role=null)
 {
  ...
  var items=q.ToList();return Ok(new PagedResult<UserView>(items.Skip((page-1)*pageSize).Take(pageSize).Select(ToView).ToList(),items.Count,page,pageSize));
 }
 ...
 private static UserView ToView(ModernCRM.Auth.Domain.Users.AuthUser x)=>new(x.Id,x.TenantId.Value,x.Username.Value,x.FirstName,x.LastName,x.Email.Value,x.Role.ToString(),x.IsActive,x.IsDeleted,x.IntegrationStatus,x.IntegrationError,DateTime.UtcNow);
 public sealed record CreateUserRequest(string? TenantId,string? Username,string? Email,string? FirstName,string? LastName,string? Password,string? Role);
 public sealed record UpdateUserRequest(string? Email,string? FirstName,string? LastName,string? Role,bool IsActive);
 public sealed record UserView(int Id,string TenantId,string Username,string FirstName,string LastName,string Email,string Role,bool IsActive,bool IsDeleted,string IntegrationStatus,string? IntegrationError,DateTime CreatedAt);
