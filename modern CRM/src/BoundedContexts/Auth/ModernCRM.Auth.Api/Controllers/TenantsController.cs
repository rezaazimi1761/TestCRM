...
public sealed class TenantsController(AuthDbContext db,ServiceInstanceStore instances):ControllerBase
{
 ...
 [HttpPost] public async Task<IActionResult> Create(Payload r,CancellationToken ct){if(string.IsNullOrWhiteSpace(r.Slug)||string.IsNullOrWhiteSpace(r.DisplayName)||r.ServiceInstanceId==Guid.Empty)return BadRequest(new{message="Slug, display name and service instance are required."});if(db.Tenants.Any(x=>x.TenantId.Value==r.Slug&&!x.IsDeleted))return Conflict(new{message="Tenant already exists."});var t=Tenant.Create(TenantId.Create(r.Slug),r.DisplayName,r.ServiceInstanceId);t.GetType().GetProperty("Id")!.SetValue(t,db.NextTenantId());db.Tenants.Add(t);await db.SaveChangesAsync(ct);return CreatedAtAction(nameof(Get),new{slug=t.TenantId.Value},t.Id);}
 [HttpPut("{slug}")] public async Task<IActionResult> Update(string slug,Payload r,CancellationToken ct){var t=db.Tenants.FirstOrDefault(x=>x.TenantId.Value==slug&&!x.IsDeleted);if(t is null)return NotFound();if(!string.IsNullOrWhiteSpace(r.DisplayName))t.Rename(r.DisplayName);if(r.ServiceInstanceId!=Guid.Empty)t.MoveToServiceInstance(r.ServiceInstanceId);if(r.IsActive==true)t.Activate();else if(r.IsActive==false)t.Deactivate();await db.SaveChangesAsync(ct);return NoContent();}
 ...
 public sealed record Payload(string? Slug,string? DisplayName,Guid ServiceInstanceId,string? Description,bool? IsActive);
