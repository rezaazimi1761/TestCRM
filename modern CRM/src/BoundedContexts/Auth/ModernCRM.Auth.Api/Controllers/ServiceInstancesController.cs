...
public sealed class ServiceInstancesController(ServiceInstanceStore store,AuthDbContext db):ControllerBase
{
 ...
 [HttpPost] public IActionResult Create(Payload r){if(string.IsNullOrWhiteSpace(r.Name)||string.IsNullOrWhiteSpace(r.ApiUrl))return BadRequest(new{message="Name and API URL are required."});lock(store.SyncRoot){var x=new ServiceInstanceModel{Id=Guid.NewGuid(),Name=r.Name,ApiUrl=r.ApiUrl,Description=r.Description,IsActive=true,CreatedAt=DateTime.UtcNow};store.Items.Add(x);return CreatedAtAction(nameof(Get),new{id=x.Id},x.Id);}}
 [HttpPut("{id:guid}")] public IActionResult Update(Guid id,Payload r){lock(store.SyncRoot){var x=store.Items.FirstOrDefault(x=>x.Id==id);if(x is null)return NotFound();x.Name=r.Name??x.Name;x.ApiUrl=r.ApiUrl??x.ApiUrl;x.Description=r.Description;x.IsActive=r.IsActive??x.IsActive;return NoContent();}}
 ...
 public sealed record Payload(string? Name,string? ApiUrl,string? Description,bool? IsActive);
}