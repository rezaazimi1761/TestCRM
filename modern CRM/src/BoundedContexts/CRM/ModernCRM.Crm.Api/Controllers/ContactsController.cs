...
public sealed class ContactsController(FrontendCrmStore store) : ControllerBase
{
    ...
    [HttpPost]
    public IActionResult Create(ContactPayload r)
    {
        lock (store.SyncRoot)
        {
            if (string.IsNullOrWhiteSpace(r.FirstName) || string.IsNullOrWhiteSpace(r.LastName) || string.IsNullOrWhiteSpace(r.Email)) 
                return BadRequest(new { message = "First name, last name and email are required." }); 
            var x = new ContactModel { Id = store.NextId(), TenantId = FrontendApi.Tenant(User), FirstName = r.FirstName, LastName = r.LastName, Email = r.Email, Phone = r.Phone, Company = r.Company, JobTitle = r.JobTitle, Notes = r.Notes, AccountId = r.AccountId };
            store.Contacts.Add(x); 
            return CreatedAtAction(nameof(Get), new { id = x.Id }, x.Id);
        }
    }
    [HttpPut("{id:int}")]
    public IActionResult Update(int id, ContactPayload r) { lock (store.SyncRoot) { var x = store.Contacts.FirstOrDefault(x => x.Id == id && x.TenantId == FrontendApi.Tenant(User) && !x.IsDeleted); if (x is null) return NotFound(); x.FirstName = r.FirstName; x.LastName = r.LastName; x.Email = r.Email; x.Phone = r.Phone; x.Company = r.Company; x.JobTitle = r.JobTitle; x.Notes = r.Notes; x.AccountId = r.AccountId; return NoContent(); } }
    [HttpDelete("{id:int}")]
